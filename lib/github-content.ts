...
function getGitHubConfig() {
  ...
  const branches = Array.from(
    new Set(
      [process.env.GITHUB_BRANCH?.trim(), process.env.VERCEL_GIT_COMMIT_REF?.trim()].filter(
        (value): value is string => Boolean(value)
      )
    )
  );

  if (!token || !owner || !repo || branches.length === 0) {
    return null;
  }
...
async function fetchGitHubFileForBranch(
...
async function fetchGitHubFile() {
  ...
  for (const branch of config.branches) {
    const file = await fetchGitHubFileForBranch(config, branch);
...
export async function updateContentOnGitHub(content: EditableSiteContent) {
  ...
  if (!existing?.sha) {
    throw new Error(
      `Unable to read the existing content file from GitHub. Checked branches: ${config.branches.join(
        ", "
      )}.`
    );
  }
